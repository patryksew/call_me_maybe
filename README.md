...
- Before sampling, autocompletion is attempted when there is only one valid transition in the FSM.
  This reduces the amount of tokens generated by the model and improves performance.
- After autocompletion is done (if any), the last token is removed from the output. This is done
  because LLMs have their own way to tokenize text; without this step, quality of output drops
  significantly.
